...
const SupervisorReportModal = ({ isOpen, onClose, intern }) => {
  ...
  const handleApprove = async (reportId, status, comment = '') => {
    ...
    try {
      ...
      const finalComment = comment || `Approved by Supervisor on ${new Date().toLocaleDateString()}`;
      ...
      const response = await fetch(`${API_BASE_URL}/api/daily-logs/${reportId}/supervisor-approve`, {
        ...
        body: JSON.stringify({
          ...
          supervisor_comment: finalComment,
        }),
        credentials: 'include',
      });
                ...
                >
                  ...
                  {commentingReportId === report.id && (
                    <div className="mb-6 p-5 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200 animate-in slide-in-from-top duration-300">
                      <label className="font-bold text-gray-800 text-base block mb-3 flex items-center gap-2">
                        <MessageSquare size={18} className="text-blue-600" /> Add Supervisor Comment:
                      </label>
                      <textarea
                      ...
                      <div className="flex gap-3 mt-4">
                        <button
                          onClick={() => handleApprove(report.id, 'Approved', supervisorComment)}
                          disabled={submitting}
                  ...
                  {report.supervisor_comment && (
                    <div className="mt-6 pt-6 border-t-2 border-gray-200">
                      <label className="font-bold text-gray-800 text-base block mb-3 flex items-center gap-2">
                        <MessageSquare size={18} className="text-blue-600" /> Supervisor Comment:
                      </label>
